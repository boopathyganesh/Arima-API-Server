...
exports.getProductCategoryByShopId = async (req, res) => {
    ...
    try {
        const products = await ProductCategory.find({vendor:shopId}).exec();
        res.status(200).send({
            status: true,
...
exports.updateProductCategoryById = async (req, res) => {
    try {
        ...
        if (updateData.status === 0) {
            const updatedProductCategory = await ProductCategory.findByIdAndUpdate(productCategoryId, updateData, { new: true }).exec();
            await Product.updateMany({ product_category: productCategoryId }, { status: 0 });
            ...
            const updatedProductCategory = await ProductCategory.findByIdAndUpdate(productCategoryId, updateData, { new: true }).exec();
            await Product.updateMany({ product_category: productCategoryId }, { status: 1 });
            if (!updatedProductCategory) {
                return res.status(404).send({ status: false, message: 'Product Category not found' });
            };
            res.status(200).send({ status: true, message: 'Product Category Updated', data: updatedProductCategory });
